...
# Zipf/power-law exponent; higher => heavier skew
S_PARAM = 0.35

# CIFAR-100 totals and per-client minimums
TRAIN_TOTAL = 50_000
TEST_TOTAL = 10_000
MIN_TRAIN = 500
MIN_TEST = 100
...
def _sizes_powerlaw(
    n_total: int, n_clients: int, min_size: int, s: float
) -> list[int]:
...
def load_data(
    ...
    """Load CIFAR100 partition with quantity-skewed (power-law) sizes."""
    global fds
    if fds is None:
