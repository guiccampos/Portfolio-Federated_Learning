...
def load_data(partition_id: int,
    ...
    size_distribution = make_sizes(total_samples=TRAIN_TOTAL, num_clients=num_partitions,min_size=MIN_TRAIN)

    global fds
    if fds is None:
        # IID partitioners for train and test
        train_partitioner = SizePartitioner(partition_sizes=size_distribution)
...
def make_sizes(total_samples: int, 
                num_clients: int, 
                min_size: int
                ) -> list[int]:

    # Ensures that every client has at least <MIN_TOTAL> number of samples
    remaining = total_samples - min_size * num_clients
    if remaining == 0:
        return [min_size] * num_clients
        
    ranks = np.arange(1, num_clients + 1, dtype=float)    # 1..N
    weights = ranks ** (-SKEW_KNOB)                              # Zipf weights
