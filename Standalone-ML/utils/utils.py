...
def compute_cosine_similarity(client_results: list[ClientTrainResult],
    ...
    return client_contribution, importance_coefficients, flattened_aggregated_gradient, flattened_aggregated_gradient_norm, cosine_similarity_computation_time, importance_coefficient_normalization_time
...
def compute_gradient_reward_max_norm(normalized_client_results: list[ClientTrainResult],
                            aggregated_normalized_gradients: list[npt.NDArray[np.generic]],
                            importance_coefficients: dict[int, float],
                            beta: float,
                            eps: float, 
    ...
    # Total number of scalar parameters across all layers
    D = int(flatten(aggregated_normalized_gradients).size)
...
def compute_gradient_reward_min_max_norm(normalized_client_results: list[ClientTrainResult],
                                        aggregated_normalized_gradients: list[npt.NDArray[np.generic]],
                                        importance_coefficients: dict[int, float],
                                        eps: float,
                                        q_min: float = 0.2,         # Maybe change this as well
                                        q_max: float = 1.0,         # Maybe change this as well
                                        delta_guard: float = 1e-3   # Maybe change this to 5e-3 or lower values if there's earli round instability
                                        ) -> tuple[
                                            dict[int, list[npt.NDArray[np.generic]]],   # v_by_client
                                            dict[int, float],                           # q_by_client
                                            dict[int, float],                           # v_non_zero_frac_by_client
    ...
    tanh_vals_computation_start_time = perf_counter()
    r_max: float = max(importance_coefficients.values())
    r_min: float = min(importance_coefficients.values())
    delta_r = r_max - r_min
    delta_q: float = float(q_max - q_min)
    ...
    for result in normalized_client_results:
        client_id = result["client_id"]
        r_i: float = importance_coefficients[client_id]

        if delta_r < delta_guard:
            q_i = q_max
        else:
            q_i = q_min + delta_q * (r_i - r_min) / (delta_r + eps)

        q_i: float = float(np.clip(q_i, q_min, q_max))
        q_by_client[client_id] = q_i

        masked_layers, nonzero_fraction, sparsity, kept_count, total_count = mask_grad_update_per_layer_by_percentile_for_analysis(
            gradient=aggregated_normalized_gradients,
            mask_percentile=q_i,
        )
        v_by_client[client_id] = masked_layers
    ...
    return v_by_client, q_by_client, v_non_zero_frac_by_client, v_sparsity_by_client, tahn_vals_computation_time, compute_gradient_reward_time
...
def accumulate_gradients(original: list[npt.NDArray[np.generic]], sparsified: list[npt.NDArray[np.generic]]) -> list[npt.NDArray[np.generic]]:
    return [gradient - masked for gradient, masked in zip(original, sparsified)]
