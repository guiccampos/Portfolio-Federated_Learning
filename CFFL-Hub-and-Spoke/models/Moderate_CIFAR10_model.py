...
def train(net: nn.Module, trainloader: DataLoader, epochs: int, device: torch.device | str, lr: float)-> tuple[float, float, list[npt.NDArray[np.generic]]]:
    ...
    parameter_update = parameter_update_fn(final_model, initial_model)

    server_like = model_to_numpy(net)                 # uses state_dict ordering
    assert len(server_like) == len(parameter_update)

    for a, b in zip(server_like, parameter_update):
...
def parameter_update_fn(final_model: nn.Module, initial_model: nn.Module) -> list[np.ndarray]:
    f_sd = final_model.state_dict()
    i_sd = initial_model.state_dict()

    update: list[np.ndarray] = []
    for k in f_sd.keys():
        f = f_sd[k].detach().cpu().numpy()
        i = i_sd[k].detach().cpu().numpy()

        # only float tensors should be updated; if you ever re-add BN counters, skip ints
        if np.issubdtype(f.dtype, np.floating):
            update.append(f - i)
        else:
            update.append(np.zeros_like(f))  # keep unchanged
